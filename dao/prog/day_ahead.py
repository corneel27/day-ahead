    ...
    def calc_optimum(
        ...
        for b in range(B):
            ...
            opt_low_lvl = float(
                self.get_setting_state(
                    "optimal lower level", self.battery_options[b], default=lower_limit[b])
            )
        ...
        if not self.hp_enabled:
            ...
            if self.hp_adjustment == "on/off":
                ...
                logging.info(
                    f"Maximaal te produceren hoeveelheid warmte: {max_heat_prod} kWh"
                )
