...
class DaCalc(DaBase):
    ...
    def _resolve_grid_balance_switch_entity_id(self):
        entity_id = self._get_option("entity balance switch", self.grid)
        if entity_id is not None:
            return entity_id

        battery_options = self._get_primary_battery_options()
        fallback_entity_id = self._get_option("entity balance switch", battery_options)
        if fallback_entity_id is not None:
            logging.info(
                "Grid balance-switch ontbreekt in grid-config; fallback via battery.entity balance switch"
            )
    ...
    def calc_optimum(
        ...
        try:
            ...
            if self.debug:
                ...
                balance_entity_id = self._resolve_grid_balance_switch_entity_id()
                if balance_entity_id is not None:
                    self.set_state(balance_entity_id, balance_state)
                else:
                    logging.info(
                        "Geen grid balance-switch entity geconfigureerd; status niet naar HA geschreven"
                    )
