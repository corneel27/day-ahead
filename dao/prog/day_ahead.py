    ...
    def calc_optimum(
        ...
        if not self.hp_enabled:
            ...
            if entity_heat_produced is not None:
                ...
                heat_produced = 0
            heat_needed = max(
                0.0, degree_days * degree_days_factor - heat_produced
            )  # heat needed
            ...
            if self.hp_adjustment == "on/off":
                ...
                if self.hp_heat_demand:
                    logging.info(f"Warmtepomp met on/off regeling wordt ingepland")
                    ...
                    # Add the vars
                    c_hp = [
                        model.add_var(var_type=CONTINUOUS, lb=0, ub=10)
                        for _ in range(U)
                    ...
                    for u in range(U):
                        model += (
                            c_hp[u] == hp_power * hp_on[u] * hour_fraction[u]
