...
class Report:
    ...
    def get_sensor_data(self, sensor: str, vanaf: datetime.datetime, tot: datetime.datetime,
        ...
        query = select(
            *columns
            ...
            t1.join(t2, t2.c.start_ts == t1.c.start_ts + 3600)
    ...
    @staticmethod
    def aggregate_data(df_raw: pd.DataFrame, col_name: str, agg: str = "uur") -> pd.DataFrame:
        df_raw['tot'] = df_raw.apply(lambda x: datetime.datetime.fromtimestamp(x['tijd']), axis=1)
        if len(df_raw) > 0:
            # Extract year and month or day
            if agg == "maand":
                df_raw['maand'] = df_raw['start_ts_t2'].dt.to_period('M')
                df_aggregated = df_raw.groupby('maand').agg(
                    maand=('start_ts_t2', lambda x: f"{x.dt.year.iloc[0]}-{x.dt.month.iloc[0]:2}"),
    ...
    def consolidate_data(self, _start=None, _end=None) -> None:
        ...
        for code, categorie in itertools.chain(self.grid_dict.items()):  # self.energy_balance_dict.items()):
            ...
            for row in data.itertuples():
                db_row = [str(int(row.tijd.timestamp())), code, float(row.value), row.tijd]
                # print(db_row)
            ...
            if source == "all" or source == "ha":
                ...
                for sensor in self.report_options["entities grid consumption"]:
                    if count == 0:
                        ...
                        df_2 = self.get_sensor_data(sensor, last_moment, tot, "consumption", "uur")
                ...
                for sensor in self.report_options["entities grid production"]:
                    df_p = self.get_sensor_data(sensor, last_moment, tot, "production", "uur")
    ...
    def get_field_data(self, field: str, periode: str):
        ...
        df = self.db_da.get_column_data('values', field, start=period["vanaf"], end=period["tot"])
