...
class CheckDB(DaBase):
    def __init__(self, file_name: str | None = None):
        ...
        self.db_url = DBmanagerObj.db_url(db_dialect=db_da_engine, db_name=db_da_name,
                                          db_server=db_da_server, db_user=db_da_user,
                                          db_password=db_da_password, db_port=db_da_port,
                                          db_path=db_da_path)
        if not sqlalchemy_utils.database_exists(self.db_url):
            sqlalchemy_utils.create_database(self.db_url)
        try:
            self.db_da = DBmanagerObj(db_dialect=db_da_engine, db_name=db_da_name,
                                      db_server=db_da_server, db_user=db_da_user,
                                      db_password=db_da_password, db_port=db_da_port,
    ...
    def check_db_da(self):
        ...
        if l_version < n_version:
            moment = datetime.datetime.fromtimestamp(round(datetime.datetime.now().timestamp()))
            insert_query = insert(version_table).values(moment=moment, value=self.version)
            with self.engine.connect() as connection:
        ...
        if l_version <= 472:
            ...
            values_tabel = Table(
                ...
                metadata,
                Column('id', Integer, primary_key=True, autoincrement=True),
                Column('variabel', Integer, ForeignKey("variabel.id",
                                                       ondelete="CASCADE"),
                       nullable=False),
