...
class DayAheadOpt(hass.Hass):
    ...
    def __init__(self, file_name=None):
        ...
        print("Day Ahead Optimalisering gestart op:",
    ...
    def calc_optimum(self, show_graph=False):
        ...
        for b in range(B):
            ...
            eff_dc_to_bat.append(
                float(self.battery_options[b]["dc_to_bat efficiency"]))  # fractie van 1
            eff_bat_to_dc.append(
                float(self.battery_options[b]["bat_to_dc efficiency"]))  # fractie van 1
            ...
            for s in range(pv_dc_num[b]):
                ...
                for u in range(U):
                    ...
                    prod_dc = self.meteo.calc_solar_rad(self.battery_options[b]["solar"][s], int(
                        tijd[u].timestamp()), global_rad[u]) * pv_yield
                    efficiency = 1
        ...
        pv_dc_on_off = [[[model.add_var(var_type=BINARY) for u in range(
            U)] for s in range(pv_dc_num[b])] for b in range(B)]
        pv_prod_dc_sum = [[model.add_var(
            var_type=CONTINUOUS, lb=0, ub=max_charge_power[b]) for u in range(U)] for b in range(B)]
        ac_to_dc = [[model.add_var(var_type=CONTINUOUS, lb=0, ub=max_charge_power[b])
                     for u in range(U)] for b in range(B)]
        dc_from_ac = [[model.add_var(
            var_type=CONTINUOUS, lb=0, ub=max_charge_power[b]) for u in range(U)] for b in range(B)]
        ...
        ac_to_dc_on = [[model.add_var(var_type=BINARY)
                        for u in range(U)] for b in range(B)]
        ...
        ac_from_dc = [[model.add_var(var_type=CONTINUOUS, lb=0, ub=max_discharge_power[b])
                       for u in range(U)] for b in range(B)]
        dc_to_ac_on = [[model.add_var(var_type=BINARY)
                        for u in range(U)] for b in range(B)]
        ...
        ac_to_dc_st_on = [[[model.add_var(var_type=BINARY) for u in range(
            U)] for cs in range(CS[b])] for b in range(B)]
        dc_to_ac_st_on = [[[model.add_var(var_type=BINARY) for u in range(
            U)] for ds in range(DS[b])] for b in range(B)]
        ...
        for b in range(B):
            for u in range(U):
                ...
                for cs in range(CS[b]):
                    model += ac_to_dc_st[b][cs][u] <= self.battery_options[b]["charge stages"][cs]["power"] * \
                        ac_to_dc_st_on[b][cs][u]/1000
                for cs in range(CS[b])[1:]:
                    model += ac_to_dc_st[b][cs][u] >= self.battery_options[b]["charge stages"][cs -
                                                                                               1]["power"] * ac_to_dc_st_on[b][cs][u]/1000
                model += ac_to_dc[b][u] == xsum(ac_to_dc_st[b][cs][u]
                                                for cs in range(CS[b]))
                model += (xsum(ac_to_dc_st_on[b][cs][u]
                          for cs in range(CS[b]))) <= 1
                model += dc_from_ac[b][u] == xsum(ac_to_dc_st[b][cs][u] * self.battery_options[b]
                                                  ["charge stages"][cs]["efficiency"] for cs in range(CS[b]))
                # ontladen
                for ds in range(DS[b]):
                    model += dc_to_ac_st[b][ds][u] <= self.battery_options[b]["discharge stages"][ds]["power"] * \
                        dc_to_ac_st_on[b][ds][u]/1000
                for ds in range(DS[b])[1:]:
                    model += dc_to_ac_st[b][ds][u] >= self.battery_options[b]["discharge stages"][ds -
                                                                                                  1]["power"] * dc_to_ac_st_on[b][ds][u]/1000
                model += dc_to_ac[b][u] == xsum(dc_to_ac_st[b][ds][u]
                                                for ds in range(DS[b]))
                model += (xsum(dc_to_ac_st_on[b][ds][u]
                          for ds in range(DS[b]))) <= 1
                model += ac_from_dc[b][u] == xsum(dc_to_ac_st[b][ds][u] / self.battery_options[b]
        ...
        for b in range(B):
            ...
            min_soc_end_opt = float(self.get_state(
                self.battery_options[b]["entity min soc end opt"]).state)
            max_soc_end_opt = float(self.get_state(
                self.battery_options[b]["entity max soc end opt"]).state)
            model += soc[b][U] >= max(opt_low_level[b] / 2, min_soc_end_opt)
            model += soc[b][U] <= max_soc_end_opt
            for u in range(U):
                model += soc[b][u + 1] == soc[b][u] + (dc_to_bat[b][u] * eff_dc_to_bat[b] / one_soc[b]) - (
                    (bat_to_dc[b][u] / eff_bat_to_dc[b]) / one_soc[b])
                model += pv_prod_dc_sum[b][u] == xsum(
                    pv_prod_dc[b][s][u] * pv_dc_on_off[b][s][u] for s in range(pv_dc_num[b]))
                model += dc_from_ac[b][u] + bat_to_dc[b][u] + \
                    pv_prod_dc_sum[b][u] == ac_from_dc[b][u] + dc_to_bat[b][u]
                model += dc_from_ac[b][u] <= ac_to_dc_on[b][u] * \
                    max_charge_power[b]
                model += dc_to_ac[b][u] <= dc_to_ac_on[b][u] * \
                    max_discharge_power[b]
        ...
        self.boiler_present = self.boiler_options["boiler present"].lower(
        ) == "true"
        if not self.boiler_present:
            ...
            cop_boiler = 3
            boiler_temp = [model.add_var(var_type=CONTINUOUS, lb=20, ub=20) for u in range(
                U + 1)]  # end temp boiler
            c_b = [model.add_var(var_type=CONTINUOUS, lb=0, ub=0)
                   for u in range(U)]  # consumption boiler
            model += xsum(boiler_on[j] for j in range(U)) == 0
            print("Geen boiler aanwezig")
        else:
            # 50 huidige boilertemperatuur ophalen uit ha
            boiler_act_temp = float(self.get_state(
                self.boiler_options["entity actual temp."]).state)
            boiler_setpoint = float(self.get_state(
                self.boiler_options["entity setpoint"]).state)
            boiler_hysterese = float(self.get_state(
                self.boiler_options["entity hysterese"]).state)
            ...
            boiler_start = int(
                max(0, min(23, int((boiler_act_temp - boiler_bovengrens) / boiler_cooling))))

            # tijdstip index waarop boiler nog aan kan
            # (41-40)/0.4=2.5
            boiler_end = int(
                min(U - 1, max(0, int((boiler_act_temp - boiler_ondergrens) / boiler_cooling))))

            boiler_temp = [
                model.add_var(var_type=CONTINUOUS, lb=min(
                    boiler_act_temp, boiler_setpoint - boiler_hysterese - 10), ub=boiler_setpoint + 10)
        ...
        for e in range(EV):
            ...
            try:
                plugged_in = self.get_state(
                    self.ev_options[e]["entity plugged in"]).state == "on"
            ...
            try:
                position = self.get_state(
                    self.ev_options[e]["entity position"]).state
            except:
                position = "away"
            ev_position.append(position)
            try:
                soc_state = float(self.get_state(
                    self.ev_options[e]["entity actual level"]).state)
            except:
                soc_state = 100.0
            actual_soc.append(soc_state)
            wished_level.append(float(self.get_state(
                self.ev_options[e]["charge scheduler"]["entity set level"]).state))
            ready_str = self.get_state(
                self.ev_options[e]["charge scheduler"]["entity ready datetime"]).state
            if len(ready_str) > 9:
                # dus met datum en tijd
                ready = datetime.datetime.strptime(
                    ready_str, '%Y-%m-%d %H:%M:%S')
            else:
                ready = datetime.datetime.strptime(ready_str, '%H:%M:%S')
                ready = datetime.datetime(
                    now_dt.year, now_dt.month, now_dt.day, ready.hour, ready.minute)
                if (ready.hour == now_dt.hour and ready.minute < now_dt.minute) or (ready.hour < now_dt.hour):
                    ready = ready + datetime.timedelta(days=1)
            max_ampere = self.get_state(
                self.ev_options[e]["entity max amperage"]).state
            try:
                max_ampere = float(max_ampere)
            except ValueError:
                max_ampere = 10
            charge_three_phase = self.ev_options[e]["charge three phase"].lower(
            ) == "true"
            ...
            print("\nInstellingen voor laden van EV:",
                  self.ev_options[e]["name"])
            ...
            energy_needed.append(
                ev_capacity * (wished_level[e] - actual_soc[e]) / 100)  # in kWh
            # uitgedrukt in aantal uren; bijvoorbeeld 1,5
            time_needed = energy_needed[e] / max_power[e]
        ...
        charger_on = [[model.add_var(var_type=BINARY)
                       for u in range(U)] for e in range(EV)]
        ...
        for e in range(EV):
            if (energy_needed[e] > 0) and (ready_u[e] < U):
                ...
                for u in range(ready_u[e] + 1):
                    model += c_ev[e][u] <= charger_on[e][u] * \
                        hour_fraction[u] * max_power[e]
                    max_beschikbaar += hour_fraction[u] * max_power[e]
                for u in range(ready_u[e] + 1, U):
                    model += charger_on[e][u] == 0
                    model += c_ev[e][u] == 0
                model += xsum(charger_on[e][j]
                              for j in range(ready_u[e] + 1)) == hours_needed[e]
        ...
        # instelbaar maken?
        # levering
        c_l = [model.add_var(var_type=CONTINUOUS, lb=0, ub=20)
               for u in range(U)]
        # teruglevering
        c_t_total = [model.add_var(
            var_type=CONTINUOUS, lb=0, ub=20) for u in range(U)]
        c_t_w_tax = [model.add_var(
            var_type=CONTINUOUS, lb=0, ub=20) for u in range(U)]
        ...
        if salderen:
            ...
            c_t_no_tax = [model.add_var(
                var_type=CONTINUOUS, lb=0, ub=10) for u in range(U)]
        ...
        self.heater_present = self.heating_options["heater present"].lower(
        ) == "true"
        if not self.heater_present:
            c_hp = [model.add_var(var_type=CONTINUOUS, lb=0, ub=0)
                    for u in range(U)]  # elektriciteitsverbruik in kWh/h
            p_hp = None
            h_hp = None
        else:
            ...
            heat_produced = float(self.get_state(
                "sensor.daily_heat_production_heating").state)
            heat_needed = max(
                0.0, degree_days * degree_days_factor - heat_produced)  # heet needed
            ...
            p_hp = [[model.add_var(var_type=CONTINUOUS, lb=0, ub=stages[s]["max_power"])
                     for u in range(U)] for s in range(S)]

            # schijven aan/uit, iedere schijf kan maar een keer in een uur
            hp_on = [[model.add_var(var_type=BINARY)
                      for u in range(U)] for s in range(S)]
            ...
            h_hp = [model.add_var(var_type=CONTINUOUS, lb=0, ub=10000)
                    for u in range(U)]
            ...
            for u in range(U):
                ...
                model += (xsum(hp_on[s][u]
                          for s in range(S))) + boiler_on[u] == 1
        ...
        for u in range(U):
            model += c_l[u] == c_t_total[u] + b_l[u] + \
                ...
                c_hp[u] - xsum(pv_ac[s][u] for s in range(solar_num))
        ...
        if model.num_solutions:  # er is een oplossing
            ...
            if self.heater_present:
                ...
                for u in range(U):
                    print("{:2.0f}".format(uur[u]), "{:6.4f}".format(pl[u]), "{:6.0f}".format(p_hp[0][u].x), "{:6.0f}".format(p_hp[1][u].x),
                          ...
                          "{:6.0f}".format(p_hp[3][u].x), "{:6.0f}".format(
                              p_hp[4][u].x), "{:6.0f}".format(p_hp[5][u].x),
                          "{:6.0f}".format(p_hp[6][u].x), "{:6.0f}".format(
                              p_hp[7][u].x), "{:6.2f}".format(h_hp[u].x),
                ...
                for u in range(U):
                    ...
                    for s in range(pv_dc_num[b]):
                        pv_prod += pv_dc_on_off[b][s][u].x * \
                            pv_prod_dc[b][s][u]
                    row = [uur[u], dc_from_ac[b][u].x, c_stage, ac_to_dc_eff, dc_to_ac[b][u].x, d_stage,
                           dc_to_ac_eff, dc_to_bat[b][u].x, bat_to_dc[b][u].x, pv_prod, soc[b][u+1].x]
                    df_accu[b].loc[df_accu[b].shape[0]] = row
                df_accu[b].loc['total'] = df_accu[b].select_dtypes(
                    numpy.number).sum()
            ...
            for u in range(U):
                ...
                row = row + [c_l[u].x, c_t_w_tax[u].x, c_t_no_tax[u].x, b_l[u],
                             c_b[u].x, c_hp[u].x, c_ev_sum[u], solar_hour_sum[u], c_l[u].x *
                             pl[u], -c_t_w_tax[u].x * pt[u],
                             -c_t_no_tax[u].x * pt_notax[u], -c_t_w_tax[u].x *
                             pt[u] - c_t_no_tax[u].x * pt_notax[u],
                             boiler_temp[u + 1].x]
...
# geeft error, bovendien als je elders aan de laadpaal staat moet ie doorgaan!
                ...
                for b in range(B):
                    ...
                    netto_vermogen = int(
                        1000 * ((ac_to_dc[b][0].x - dc_to_ac[b][0].x) / hour_fraction[0]))
            ...
            plt.savefig("../data/images/optimum" +
                        datetime.datetime.now().strftime("%H%M") + ".png")
            if show_graph:
                plt.show()
            plt.close()
    ...
    def scheduler(self):
        ...
        while True:
            ...
            next_min = t - \
                datetime.timedelta(minutes=-1, seconds=t.second,
