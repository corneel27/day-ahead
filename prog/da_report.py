...
class Report ():
    ...
    def __init__(self):
        ...
        tot = datetime.datetime(now.year, now.month,
        ...
        self.periodes.update(create_dict(
            "dit contractjaar", vanaf, tot, "maand"))
        return
    ...
    def add_col_df(self, add_from, add_to, col_name):
        ...
        for row in add_from.itertuples():
            add_to.at[row.tijd, col_name] = add_to.at[row.tijd,
                                                      col_name] + add_from.at[row.tijd, col_name]
        return add_to
    ...
    def recalc_df_ha(self, org_data_df: pd.DataFrame, interval: str) -> pd.DataFrame:
        ...
        """
        fi_df = pd.DataFrame(
            columns=[interval, "vanaf", "tot", "consumed", "produced", "cost", "profit"])
        ...
        for row in org_data_df.itertuples():
            ...
            col_1 = row.consumed
            col_2 = row.produced
            col_3 = (row.consumed * (row.price + taxes_l + ol_l)) * \
                (1 + btw / 100)
            col_4 = (row.produced * (row.price + taxes_t + ol_t)) * \
                (1 + btw / 100)
            fi_df.loc[fi_df.shape[0]] = [tijd_str, row.tijd, row.tijd +
                                         datetime.timedelta(hours=1), col_1, col_2, col_3, col_4]
        if interval != "uur":
        ...
        if last_moment < tot:
            ...
            for sensor in self.report_options["entities grid consumption"]:
                if count == 0:
                    df_ha = self.get_sensor_data(
                        sensor, last_moment + datetime.timedelta(hours=-1), tot, "consumed")
            ...
            for sensor in self.report_options["entities grid production"]:
                df_p = self.get_sensor_data(
                    sensor, last_moment + datetime.timedelta(hours=-1), tot, "produced")
...
#        report_df = report_df.reset_index()
        ...
        for row in report_df.itertuples():
            ...
            if math.isnan(row.cost):
                col_4 = (row.consumed * (row.price + taxes_l + ol_l)
                         ) * (1 + btw / 100)
            else:
                col_4 = row.cost
            if math.isnan(row.profit):
                col_5 = (row.produced * (row.price + taxes_t + ol_t)
                         ) * (1 + btw / 100)
            else:
                col_5 = row.profit
            col_6 = col_4 - col_5
            ...
            fi_df.loc[fi_df.shape[0]] = [
                tijd_str, col_1, col_2, col_3, col_4, col_5, col_6]
        ...
        fi_df['Tarief verbr.'] = fi_df.apply(
            lambda row: row.Kosten / row.Verbruik if row.Verbruik != 0.0 else row.Verbruik, axis=1)
        fi_df['Tarief prod.'] = fi_df.apply(
            lambda row: row.Opbrengst / row.Productie if row.Productie != 0.0 else row.Productie, axis=1)
        if active_view == "tabel":
            ...
            fi_df.at[fi_df.index[-1],
                     "Tarief prod."] = row.Opbrengst/row.Productie
