...
| Symbol | Meaning |
|--------|---------|
| 📖 | Field has detailed help text (expand for details) |
...
| `entity instant start` | [EntityId](#entityid) (optional) | No | `null` | HA entity for instant start |
| `cop` | [FlexFloat](#flexfloat) | No | `3.0` | Coefficient of Performance (Unit: `ratio`) _Must be > 0, use 1.0 for resistive, 2.5-4.0 for heat pump_ |
| `cooling rate` | [FlexFloat](#flexfloat) | Yes | — | Cooling rate in degrees per hour (Unit: `°C/h`) _Must be >= 0, typically 0.5-2.0°C/h_ |
...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `power` | number | Yes | — | Power level in Watts (Unit: `W`) _Must be 0 or greater, stages should be sorted by power_ |
| `efficiency` | number | Yes | — | Efficiency at this power level (0-1) (Unit: `ratio (0-1)`) _Must be between 0 and 1_ |
...
| `power` | list[number] | Yes | — | Power consumption in watts for each time interval (Unit: `W`) _List of power values, one per interval (typically 1h each)_ |
